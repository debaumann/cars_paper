...
print(device)
seed = 7
no_attention_vit_utils.set_seed(seed)
batch_size = 1
wandb.init(project="no_attention_transformer")
dataset_root = "/cluster/scratch/cbennewitz/"
timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
images_heatmaps_path = f"run_no_attention_{timestamp}"
...
def main():
    ...
    model = no_attention_vit_utils.ViTMLPModel()
    model.to(device)

    train_data = no_attention_vit_utils.TrainData(dataset_root)
    val_data = no_attention_vit_utils.ValData(dataset_root)
    test_data = no_attention_vit_utils.TestData(dataset_root)
    ...
    for epoch in range(num_epochs):
        ...
        for sequences, hand_masks, obj_masks, labels in tqdm(train_loader):
            ...
            processed_sequences = no_attention_vit_utils.preprocess(preprocessor, sequences)
            ...
            running_hand_iou += no_attention_vit_utils.soft_iou(hand_attentions, hand_masks)
            running_obj_iou += no_attention_vit_utils.soft_iou(obj_attentions, obj_masks)
            ...
            train_top5 += no_attention_vit_utils.top_5(output, labels)
        ...
        with torch.no_grad():
            for sequences, hand_masks, obj_masks, labels in tqdm(val_loader):
                ...
                val_hand_iou += no_attention_vit_utils.soft_iou(hand_attentions, hand_masks)
                val_obj_iou += no_attention_vit_utils.soft_iou(obj_attentions, obj_masks)
                ...
                val_top5 += no_attention_vit_utils.top_5(output, labels)
        ...
        if current_best > avg_val_class_loss:
            ...
            with torch.no_grad():
                for sequences, labels in tqdm(test_loader):
                    ...
                    test_top5 += no_attention_vit_utils.top_5(output, labels)
            ...
            model_save_path = "no_attention_transformer_model_best.pth"
